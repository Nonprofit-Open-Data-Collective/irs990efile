...
dat <- 
  dat %>% 
  dplyr::filter( RDB_TABLE != "" ) %>%
  dplyr::arrange( LOCATION_CODE_FAMILY, RDB_TABLE, VARIABLE_NAME )
...
dat$DESCRIPTION <- gsub( "�", "", dat$DESCRIPTION )
```
...
dat$LOCATION_CODE_SORT <- gsub( "-EZ-", "-ZZ-", dat$LOCATION_CODE )
dat$LOCATION_CODE <- toupper( dat$LOCATION_CODE )
TABLES <- unique(dat$RDB_TABLE) |> sort()
...
for( i in TABLES )
{
  ...
  dat.sub <- 
    dat %>% 
    dplyr::filter( RDB_TABLE == i ) %>%
    arrange( LOCATION_CODE_SORT )
  ...
  # COMBINE EZ AND PC IMPERFECT VARIANTS 
  dat.sub$VARIABLE_NAME <- gsub( "_V2$", "", dat.sub$VARIABLE_NAME ) 
  ...
  d2 <- 
  dat.sub %>% 
  select( PREFIX, VARIABLE_NAME, DESCRIPTION, 
          LOCATION_CODE_FAMILY, LOCATION_CODE_SORT, SCOPE ) %>%
  ...
  d3 <- 
    ...
    select( PREFIX, VARIABLE_NAME, DESCRIPTION, 
            LOCATION_CODE, LOCATION_CODE_SORT, SCOPE ) %>% 
    arrange( LOCATION_CODE_SORT ) %>%
    select( -LOCATION_CODE_SORT )
