...
collapse_nodes <- function( lc.xpaths )
{
  ...
  for(i in 1:length(lc.xpaths)){
    ...
    this.one <- which( not.equal == T )[ 1 ]
    group.names <- d1[ this.one,  ] %>% as.character() %>% unique()
    # print(paste0("lc.xpath",lc.xpath))
    # print(group.names)
    # print(lc.xpath == group.names)
    # print(paste0("group.names",group.names))
    for(name in group.names){
      if(!(grepl("IRS990",name) | grepl(gsub("/","",lc.xpath),name))){
        name <- paste0( "//", lc.xpath, name )
        # print(paste0("test ",group.names))
        comb.names <- append(comb.names,name)
      }
      else comb.names <- append(comb.names,paste0("//",gsub("/","",lc.xpath)))
    }
